...
CONF_CONFIGO_ID = "configo"
CONF_TC_BUS_ID = "tc_bus"

configo_ns = cg.esphome_ns.namespace("configo")
ConfigoComponent = configo_ns.class_("ConfigoComponent", cg.Component)
...
CONFIG_SCHEMA = (
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(ConfigoComponent),
...
def validate_logger(config):
    ...
    if logger_conf[CONF_BAUD_RATE] == 0:
        raise cv.Invalid("Configo requires the logger baud_rate to be not 0")
    if CORE.using_esp_idf and (
        logger_conf[CONF_HARDWARE_UART] == USB_CDC
        and get_esp32_variant() == VARIANT_ESP32S3
    ):
        raise cv.Invalid(
            "Configo does not support the selected logger hardware_uart"
        )
    return config
...
async def to_code(config):
    cg.add_global(configo_ns.using)
